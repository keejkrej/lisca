...
- **CLI:** Prefer `bun lisca <dev|build|dist|typecheck|preview|install> <aligner|annotator|studio|landing|workspace> [target]` over raw `turbo` for product orchestration — `scripts/lisca.mjs`. Underneath, JS tasks go through `vp`.
...
- **Agent verification:** Playwright for web (Vite apps, Expo web-native, Electron); pymobiledevice3 for physical iOS (`bun lisca dev * ios`) — start dev servers, reproduce, and verify yourself; `docs/agents/mobile.md` for ports.
