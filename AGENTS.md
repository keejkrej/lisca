...
- **CLI:** Root scripts in `package.json` cover daily dev: `vp run dev:aligner`, `dev:annotator`, `dev:studio`, `dev:landing` (frontend + backend via `vp run --parallel`); `vp run dist:<product>` (Tauri packaging via `scripts/package-tauri.mjs`); `vp run deploy:landing` (Render deploy via `scripts/deploy-landing.mjs`); `vp run build` (all packages + web apps). Underneath, per-package tasks go through `vp run --filter <pkg> <task>`.
