...
- Generation pipeline (`bun run --cwd packages/contracts generate`, then `… rust-types`): Effect HttpApi → `openapi.json` (`OpenApi.fromApi`) + `contract.schema.json` → Rust `serde` types via `typify` into `crates/lisca/src/protocol/generated.rs` (re-exported from `protocol.rs`).
- `gen-rust-schema.ts` normalizes discriminated unions (`anyOf` + `$ref` → inline `oneOf`) so typify emits internally-tagged enums like `AlignerSource`.
