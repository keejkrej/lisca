...
- GitHub Actions runs `vp run fmt:check` and `vp run check` on pull requests and `main`. Non-draft same-repo pull requests also get an automatic Grok Build review (`.github/workflows/grok-review.yml`; setup in [`docs/agents/software-factory.md`](docs/agents/software-factory.md)). A `v*` tag publishes unsigned Studio, Aligner, and Annotator installers (macOS DMG, Windows NSIS, Linux deb) to a GitHub Release. Studio installers include the public killing ONNX. The `notebooks-release` workflow (`workflow_dispatch` from `main`, not a tag push) publishes `lisca-notebooks-X.Y.Z.zip` and tags `notebooks-v*` (not desktop installers).
