...
pub const RunOptions = struct {
    ...
    bundle_id: []const u8,
    icon_path: []const u8 = "assets/icon.ico",
    ...
    fn appInfo(self: RunOptions) zero_native.AppInfo {
        return .{
            ...
            .main_window = .{
                ...
                .default_frame = zero_native.geometry.RectF.init(0, 0, self.width, self.height),
                .resizable = true,
                .restore_state = true,
            },
        };
...
fn runMacos(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var app_info = options.appInfo();
    const store = prepareStateStore(init.io, init.environ_map, &app_info, &buffers);
    var mac_platform = try zero_native.platform.macos.MacPlatform.initWithOptions(zero_native.geometry.SizeF.init(options.width, options.height), webEngine(), app_info);
...
fn runLinux(app: zero_native.App, options: RunOptions, init: std.process.Init) !void {
    ...
    var app_info = options.appInfo();
    const store = prepareStateStore(init.io, init.environ_map, &app_info, &buffers);
    var linux_platform = try zero_native.platform.linux.LinuxPlatform.initWithOptions(zero_native.geometry.SizeF.init(options.width, options.height), webEngine(), app_info);
