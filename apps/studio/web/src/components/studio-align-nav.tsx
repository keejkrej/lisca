...
export function StudioAlignNav() {
  const nav = useStudioAlignNav();
  const disabled = createMemo(() => !nav.scan);
  const positionOptions = createMemo(() =>
    toAxisNavigationOptions(nav.alignPositions, nav.scan?.positionLabels),
  );
      ...
      <FrameNavigation
        ...
        timepoint={{
          ...
          onCommit: (nextValue) => {
            const max = Math.max(0, (nav.scan?.times?.length ?? 1) - 1);
            nav.setSelection({
              time: nav.scan?.times?.[clamp(Math.round(nextValue), 0, max)] ?? 0,
            });
          },
          onPrevious: () => {
            const max = Math.max(0, (nav.scan?.times?.length ?? 1) - 1);
