...
# Export gitignore: keep venv/tools untracked, and sibling update backups
# (notebooks/*.backup-*.ipynb). Backups stay next to the templates as real
# notebooks so Jupyter can open them.
printf '%s\n' ".venv/" ".uv/" ".tools/" ".ipynb_checkpoints/" "__pycache__/" "*.pyc" "*.pyo" "notebooks/*.backup-*.ipynb" >"$DEST/.gitignore"
