...
const IconStackLayer = (props: IconStackLayerProps) => {
  const x = () => props.x ?? 0;
  const y = () => props.y ?? 0;
  const strokeOpacity = () => (props.active ? "0.3" : "0.2");

  return (
    <g opacity={props.opacity} transform={`translate(${x()} ${y()})`}>
      <path
