...
def test_validate_slide_mapping_orders_keys_and_deduplicates_positions() -> None:
    validated = validate_slide_mapping({"2": [10, 12, 10], "0": [2, 0]})

    assert validated == {0: [0, 2], 2: [10, 12]}


def test_serialize_slide_mapping_orders_numeric_keys() -> None:
    serialized = serialize_slide_mapping({2: [10, 12], 0: [0, 2]})

    assert serialized == '{\n  "0": [\n    0,\n    2\n  ],\n  "2": [\n    10,\n    12\n  ]\n}\n'


def test_write_slide_mapping_writes_json(tmp_path: Path) -> None:
    output_path = write_slide_mapping({1: [4, 6, 8]}, tmp_path / "nested" / "slide.json")
...
def test_load_slide_mapping_reads_valid_json(tmp_path: Path) -> None:
    ...
    assert load_slide_mapping(slide_path) == {1: [4, 6, 8]}
