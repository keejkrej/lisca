...
export function useAnnotateSessionCore(options: UseAnnotateSessionCoreOptions) {
  ...
  const sessionActions: AnnotateSessionActions = useMemo(
    () => ({
  ...
  const position = useMemo(
    () => currentPosition(scanData ?? null, ui.selection.pos),
    [scanData, ui.selection.pos],
  );
  ...
  return {
    ...
    meta: { scanLoading } satisfies AnnotateSessionMeta,
    derived: {
      scan: scanData ?? null,
      position,
    },
